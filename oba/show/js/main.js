...
;(function() {
  var app = {
    ...
    createPoster: function createPoster(title, date, url, desc) {
      ...
      var highres = newUrl.replace('level3', 'level2')
      var el =
    ...
    generateUrl: function generateUrl(type, limit) {
      ...
      if (type === 'posters') {
        ...
        query =
          ...
          this.buildings[this.currentBuilding].dateRange[this.currentDate][0] +
          '"^^xsd:dateTime && ?date < "' +
    ...
    handleData: function handleData() {
      ...
      this.getData(this.generateUrl('posters')).then(function(result) {
        ...
        window.localStorage.setItem(
          _this3.buildings[_this3.currentBuilding].name.toLowerCase() +
            '_posters',
      ...
      if (
        ...
        if (
          window.localStorage.getItem(
            this.buildings[this.currentBuilding].name.toLowerCase() +
              '_background-' +
              this.currentDate +
              '-' +
              randomNum
          ...
          this.getData(this.generateUrl('background', randomNum)).then(function(
            result
          ) {
            console.log(result)
            ...
            app.elements.background.style.backgroundImage =
              'url(' + newUrl + ')'
          })
        }
        loaded = loaded + 2
        ...
        this.getData(this.generateUrl('count'))
          ...
          .then(function() {
            _this3
              .getData(
                _this3.generateUrl('background', helper.randomize(1, limit) - 1)
              )
              .then(function(result) {
                window.localStorage.setItem(
                  _this3.buildings[_this3.currentBuilding].name.toLowerCase() +
